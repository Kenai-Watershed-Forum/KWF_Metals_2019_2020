...
* In the future it will be important to be able to read in final data directly from the EPA institutional archive, in its most static version.  I found Cu, Zn, Mg, and Ca data pre-2013 there at https://www.waterqualitydata.us/portal/, but did not yet find 2014-2020 data there.
...
* Where are the rest of the years archived? Do they need to be submitted AWQMS?  Maybe use a different query?
...
# read in tabs
tbl <- read_excel(workbook, sheet = tab, skip = 2) 
...
tbl <- tbl%>%
  ...
  filter(date != "Date") %>%
  fill(site) %>%
  transform(date = as.Date(as.numeric(date), origin = "1899-12-30")) %>%
...
Show sampling sites and raw data on an ArcGIS Online map (also at https://arcg.is/0yij15)
```{r}
# add site identifier
tbl <- tbl %>%
  group_by(site) %>%
